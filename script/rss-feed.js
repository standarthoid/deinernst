...
function createEpisodeCard(episode, index, totalEpisodes) {
    ...
    const episodeNumber = episode.title.match(/#(\d+)/) 
        ...
        <div class="episode-full-card" data-episode-index="${index}">
            <div class="episode-header-row">
                <div class="episode-badges">
                    ...
                    <span class="episode-number-large">Episode #${episodeNumber}</span>
                </div>
...
function createCompactEpisodeCard(episode, index, totalEpisodes) {
    const spotifyId = extractSpotifyId(episode.description);
    const pubDate = formatDate(episode.pubDate);
...
function createLatestEpisodeCard(episode, totalEpisodes) {
    const spotifyId = extractSpotifyId(episode.description);
    const pubDate = formatDate(episode.pubDate);
...
async function displayAllEpisodes() {
    ...
    const episodesHTML = episodes.map((episode, index) => createEpisodeCard(episode, index, totalEpisodes)).join('');
...
async function displayLatestEpisode() {
    ...
    const totalEpisodes = episodes.length;
    container.innerHTML = createLatestEpisodeCard(latestEpisode, totalEpisodes);
}
...
async function displayRecentEpisodes() {
    ...
    const episodesHTML = recentEpisodes.map((episode, index) => createCompactEpisodeCard(episode, index + 1, totalEpisodes)).join('');
